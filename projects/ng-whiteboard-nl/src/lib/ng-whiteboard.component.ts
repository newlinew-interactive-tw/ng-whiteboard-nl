...
export class NgWhiteboardComponent implements AfterViewInit, OnDestroy {
  ...
  @Output() content2Engage = new EventEmitter();
  ...
  private fontStyle = 'italic 20px Arial';
  private fontColor = 'rgba(0, 0, 0, 1)'; // For fillText
  private borderWidth = 2.2;
  private borderColor = 'rgba(0, 0, 0, 1)'; // For strokeText border
  // private currentText = 'just for testing';
  private context;
  ...
  ngAfterViewInit() {
    ...
    this.subscriptionList.push(this.whiteboardService.addTextMethodCalled$.subscribe((text2Add) => this.addText(text2Add)));
    this.subscriptionList.push(this.whiteboardService.send2EngageMethodCalled$.subscribe(() => this.send2Engage()));
  ...
  private addText(text2Add: string) {
    this.drawText(
  ...
  private send2Engage() {
    ...
    const img = 'data:image/svg+xml;base64,' + btoa(unescape(encodeURIComponent(svgString)));
    this.embedImg(img);
    this.content2Engage.emit();
  }
  ...
  private embedImg (img: string) {
    const span = document.createElement('span') as HTMLSpanElement;
    span.innerHTML = img;
    span.classList.add('svg-img');
  ...
  private drawText(
    ...
    const group = this.selection
      ...
      group
        ...
        .attr('width', width)
        .text(text2Add)
        .attr('fill', this.color || this.whiteboardOptions.color );
